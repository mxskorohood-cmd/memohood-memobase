...
VENDORED VERBATIM from ``hermes-kb/rerank.py`` (v0.1.0, 2026-07-06) per
HERMES_UPGRADES.md §1.3's "вендорим копией" decision — this module never
referenced chunk/capture/collection table names (it works purely on generic
``{"text": ...}`` candidate dicts handed to it by ``retrieve.py``), so
NOTHING here needed renaming; only the module docstring/logger namespace
were touched for provenance.
...
which a live bot will exceed easily, and "reranker degraded to RRF-only" is
a normal, expected, visibly-flagged operating mode, not an error path.
...
(``memory.memohood.monthly_ceiling_usd.cohere`` — a SEPARATE ceiling from
hermes-kb's own, since the two plugins keep independent ledgers/databases).
"""
